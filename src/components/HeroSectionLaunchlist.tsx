...
export const HeroSectionLaunchlist = () => {
...
const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!referralCode) {
        ...
        let result = await addRefereeToWaitlistNoReferralCode(email, newReferralCode, nextWaitlistPosition, "Student")//don't need to return anything
        if (!result.success) {
           // handle error
    ...
    if (error || !referrer || referrer.length === 0) {
      ...
      referrerIdentityWaitingPosition = referrer[0].waitlist_position
        ...
        if (nextWaitlistPositionBonus <= 0) {
          console.log("nextWaitlistPositionBonus is smallert than 0, o just adding to end as normal without doing it")
        ...
        const { data: data2, error: error2 } = await supabase
            .from('waitlist_salary_advance')
            .insert([
              { email: email, referral_code: newReferralCode, waitlist_position: nextWaitlistPositionBonus, landing_page_referral: "Student" },
            ])
            .select()
...
//function to do "show what you could go, how often"
function samplePriceInstallements(salary, frequency) {
  switch (frequency) {
    case "daily":
      return `£${salary / frequency} every day`
    case "weekly":
      return `£${salary / frequency} every week`
    case "fortnight":
      return `£${salary / frequency} every fortnight`
  }
              ...
              <div className="flex-1 space-y-8 text-center lg:text-left">
                ...
                <div className="space-y-6">
                  <h1 className="text-4xl md:text-6xl font-black text-purple-900 leading-tight transform hover:scale-105 transition-transform duration-300">
                    ...
                    <span className="text-pink-600 underline decoration-wavy decoration-pink-400">Not Weeks Later</span>
                  </h1>
                  <h2 className="text-2xl md:text-3xl font-bold text-purple-800 leading-relaxed">
                    Receive your salary every {" "}
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-8 pt-4">
                  ...
                  <div className="flex items-center gap-2 text-purple-700 bg-white/50 px-4 py-2 rounded-full shadow-sm">
                    ...
                    <span className="font-semibold">Avoid Scams</span>
                  </div>
                  <div className="flex items-center gap-2 text-purple-700 bg-white/50 px-4 py-2 rounded-full shadow-sm">
